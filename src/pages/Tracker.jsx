...
export default function Tracker() {
  ...
  // Compute all days for the current calendar month dynamically for the GitHub grid
  const currentMonthData = useMemo(() => {
    ...
    return {
      name: current.toLocaleString('default', { month: 'long' }).toUpperCase(),
      year,
      days
    };
  }, []);
  ...
  const [modalUnit, setModalUnit] = useState('days'); // 'days' | 'months'
  const [modalQuantity, setModalQuantity] = useState('7');
  ...
  const handleCreateTracker = (e) => {
    e.preventDefault();
    if (!newTrackerName.trim() || !modalQuantity || Number(modalQuantity) <= 0) return;

    const qty = Number(modalQuantity);
    const calculatedTotalDays = modalUnit === 'months' ? qty * 30 : qty;
    ...
    setCustomTrackers([...customTrackers, {
      ...
      startDate: getLocalDateString() // Anchor points configuration track to calendar sequence
    }]);
  ...
  // Helper calculation for dynamic water square colors based on volumetric progress
  const getWaterColorClass = (dateStr) => {
    const log = dailyLogs[dateStr];
    if (!log) return 'bg-white/40 border-[#2D3A47]/10';
    
    const count = waterUnit === 'glass' 
      ? (log.waterGlasses ? log.waterGlasses.filter(Boolean).length : 0)
      : (log.waterBottles ? log.waterBottles.filter(Boolean).length : 0);
    
    if (count === 0) return 'bg-white/40 border-[#2D3A47]/10';
    
    const ratio = count / (waterUnit === 'glass' ? 8 : 2.5);
    if (ratio <= 0.25) return 'bg-blue-200 border-blue-300';
    if (ratio <= 0.50) return 'bg-blue-400 border-blue-500';
  ...
  return (
    <div className="bg-[#FFF7E6] text-[#2D3A47] min-h-screen relative w-full p-6 md:p-12 flex flex-col justify-between font-sans subpixel-antialiased overflow-x-hidden selection:bg-[#F7C8D3]">
          ...
          <div className="mt-10 backdrop-blur-2xl bg-white/25 border border-white/50 border-t-white/80 border-l-white/80 p-8 rounded-[2.5rem] shadow-[0_30px_70px_rgba(45,58,71,0.08)]">
            ...
            <div className="flex flex-col gap-5">
              {customTrackers.map(tracker => {
                // Calculate individual date checkboxes based on custom quantity configurations
                const totalDays = tracker.totalDays || 7; 
                const start = new Date(tracker.startDate || todayStr);
                ...
                return (
                  <div key={tracker.id} className="bg-white/30 border border-white/40 p-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-sm transition-all duration-300 hover:bg-white/40">
                    ...
                    <div className="flex justify-between items-center md:w-1/3">
                      <div>
                        ...
                        <p className="text-[10px] font-mono text-[#A8B58A] uppercase font-black tracking-widest mt-0.5">
                          Target Matrix: {tracker.quantity} {tracker.unit || 'days'} ({totalDays} Cells)
                        </p>
                      </div>
                      <button 
                        type="button"
                    ...
                    <div className="flex items-center justify-between md:justify-end gap-6 flex-1 w-full">
                      {/* Wrappable & scannable tracker matrix for scalable numbers of custom fields */}
                      <div className="flex flex-wrap gap-3 max-h-40 overflow-y-auto p-1 justify-start flex-1">
                        {trackerDays.map(({ dayNum, dateStr }) => {
                          const isChecked = !!dailyLogs[dateStr]?.customTasks?.[tracker.id];
                          const isToday = dateStr === todayStr;

                          return (
                            <div key={dateStr} className="flex flex-col items-center gap-1">
                              <span className={`text-[9px] font-mono font-bold ${isToday ? 'text-[#B46A72]' : 'opacity-40'}`}>D{dayNum}</span>
                              <button
                                type="button"
                                ...
                                className={`w-9 h-9 rounded-xl flex items-center justify-center text-xs font-black transition-all duration-300 border ${
                                  isChecked 
                                    ...
                                    : isToday 
                                      ? 'bg-white/50 border-white text-[#2D3A47]/40 text-center' 
                      ...
                      <button 
                        type="button"
                        onClick={() => deleteCustomTracker(tracker.id)}
                        className="hidden md:block text-[#B46A72]/50 hover:text-[#B46A72] font-mono text-lg font-black transition-colors ml-4 shrink-0"
                      >
                        [X]
                      </button>
                    </div>
      ...
      {/* MONTHLY HEATMAP REGISTRY FOOTER CONTAINER */}
      <footer className="w-full max-w-7xl mx-auto mt-14 border-t border-[#2D3A47]/10 pt-8 z-10 relative">
        <div className="backdrop-blur-2xl bg-white/25 border border-white/50 border-t-white/80 border-l-white/80 p-6 md:p-8 rounded-[2.5rem] shadow-[0_30px_70px_rgba(45,58,71,0.08)]">
          <div className="text-[11px] font-mono font-black tracking-widest mb-6 text-[#2D3A47]/60 flex items-center justify-between">
            <span>// MONTHLY CONTRIBUTION PROTOCOL</span>
            <span className="bg-[#2D3A47] text-[#FFF7E6] px-3 py-0.5 rounded-lg text-[10px] font-sans tracking-normal font-bold">
          ...
          <div className="space-y-6">
            ...
            {/* ROW 1: WATER HEATMAP METRIC (BLUE MATRIX) */}
            <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-2 border-b border-[#2D3A47]/5 pb-4">
              <div className="w-48 shrink-0">
                <div className="text-xs font-black uppercase text-[#2D3A47]">HYDRO DYNAMICS</div>
                <div className="text-[9px] font-mono uppercase tracking-wider text-blue-500/80">Fluid Intake Volumetrics</div>
              </div>
              <div className="flex flex-wrap gap-1.5 items-center overflow-x-auto py-1">
                {currentMonthData.days.map(({ dayNum, dateStr }) => (
                  <div 
                    key={`grid-water-${dateStr}`}
                    title={`Day ${dayNum}: Water Level Metric`}
                    className={`w-5 h-5 md:w-6 md:h-6 rounded-md border transition-all duration-300 select-none ${getWaterColorClass(dateStr)} ${dateStr === todayStr ? 'ring-2 ring-[#2D3A47]' : ''}`}
                  />
                ))}
              </div>
            </div>

            {/* ROW 2: EXERCISE HEATMAP METRIC (PINK MATRIX) */}
            <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-2 border-b border-[#2D3A47]/5 pb-4">
              <div className="w-48 shrink-0">
                <div className="text-xs font-black uppercase text-[#2D3A47]">KINETIC SYNC</div>
                <div className="text-[9px] font-mono uppercase tracking-wider text-[#B46A72]">Exercise Engagement Matrix</div>
              </div>
              <div className="flex flex-wrap gap-1.5 items-center overflow-x-auto py-1">
                {currentMonthData.days.map(({ dayNum, dateStr }) => {
                  const hasExercised = !!dailyLogs[dateStr]?.exercise;
                  return (
                    <div 
                      key={`grid-exercise-${dateStr}`}
                      title={`Day ${dayNum}: Kinetic Sync`}
                      className={`w-5 h-5 md:w-6 md:h-6 rounded-md border transition-all duration-300 select-none ${
                        hasExercised 
                          ? 'bg-[#B46A72] border-[#B46A72]/50 shadow-[0_4px_10px_rgba(180,106,114,0.3)]' 
                          : 'bg-white/40 border-[#2D3A47]/10'
                      } ${dateStr === todayStr ? 'ring-2 ring-[#2D3A47]' : ''}`}
                    />
                  );
                })}
              </div>
            ...
            {/* ROW 3+: SEPARATE GRID SQUARES PER CUSTOM TARGETED TASK (GREEN MATRIX) */}
            {customTrackers.map(tracker => (
              <div key={`grid-row-${tracker.id}`} className="flex flex-col xl:flex-row xl:items-center justify-between gap-2 border-b border-[#2D3A47]/5 last:border-0 pb-4 last:pb-0">
                <div className="w-48 shrink-0">
                  <div className="text-xs font-black uppercase truncate text-[#2D3A47] max-w-[11rem]">{tracker.name}</div>
                  <div className="text-[9px] font-mono uppercase tracking-wider text-[#A8B58A]">Modular Expansion Array</div>
                </div>
                <div className="flex flex-wrap gap-1.5 items-center overflow-x-auto py-1">
                  {currentMonthData.days.map(({ dayNum, dateStr }) => {
                    const isTaskCompleted = !!dailyLogs[dateStr]?.customTasks?.[tracker.id];
                    return (
                      <div 
                        key={`grid-custom-${tracker.id}-${dateStr}`}
                        title={`Day ${dayNum}: ${tracker.name}`}
                        className={`w-5 h-5 md:w-6 md:h-6 rounded-md border transition-all duration-300 select-none ${
                          isTaskCompleted 
                            ? 'bg-[#A8B58A] border-[#A8B58A]/50 shadow-[0_4px_10px_rgba(168,181,138,0.3)]' 
                            : 'bg-white/40 border-[#2D3A47]/10'
                        } ${dateStr === todayStr ? 'ring-2 ring-[#2D3A47]' : ''}`}
                      />
                    );
      ...
      <div className="fixed bottom-8 right-8 z-40">
        <button 
          ...
          className="bg-gradient-to-b from-white via-[#F7C8D3] to-[#F7C8D3] text-[#2D3A47] font-black w-15 h-15 rounded-2xl flex items-center justify-center text-2xl shadow-[0_15px_35px_rgba(180,106,114,0.25)] transition-all duration-200 hover:scale-110 active:scale-95 border border-white/60"
        >
          ＋
        </button>
      </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-[#2D3A47]/20 backdrop-blur-md z-50 flex items-center justify-center p-4 transition-all duration-300">
          <form 
            ...
            className="bg-white/70 backdrop-blur-3xl border border-white/80 p-8 w-full max-w-md rounded-[2.5rem] relative shadow-[0_40px_90px_rgba(45,58,71,0.15)]"
          >
            ...
            <input 
              ...
              className="w-full bg-white/40 border border-white/60 text-[#2D3A47] px-4 py-3.5 text-sm uppercase font-black rounded-xl focus:outline-none focus:bg-white transition-all duration-200 mb-5 shadow-inner"
              autoFocus
            />

            {/* STRATEGIC UNIT SWITCH BUTTON ARRAYS */}
            <label className="block text-xs font-mono tracking-wider uppercase font-black text-[#2D3A47]/70 mb-2">
              HORIZON METRIC TIMELINE
            </label>
            <div className="bg-white/40 border border-white/60 p-1 flex rounded-xl mb-5 shadow-[inset_0_2px_5px_rgba(0,0,0,0.03)] backdrop-blur-md">
              <button
                type="button"
                onClick={() => { setModalUnit('days'); setModalQuantity('7'); }}
                className={`flex-1 py-2 rounded-lg text-[10px] font-black tracking-wider transition-all duration-300 ${modalUnit === 'days' ? 'bg-[#2D3A47] text-white shadow-sm' : 'text-[#2D3A47]/60 hover:text-[#2D3A47]'}`}
              >
                DAYS PROTOCOL
              </button>
              <button
                type="button"
                onClick={() => { setModalUnit('months'); setModalQuantity('1'); }}
                className={`flex-1 py-2 rounded-lg text-[10px] font-black tracking-wider transition-all duration-300 ${modalUnit === 'months' ? 'bg-[#2D3A47] text-white shadow-sm' : 'text-[#2D3A47]/60 hover:text-[#2D3A47]'}`}
              >
                MONTHS PROTOCOL
              </button>
            </div>

            {/* DURATION INPUT MATRIX */}
            <label className="block text-xs font-mono tracking-wider uppercase font-black text-[#2D3A47]/70 mb-2">
              TARGET VALUE RANGE ({modalUnit.toUpperCase()})
            </label>
            <input 
              type="number" 
              min="1"
              ...
              placeholder={modalUnit === 'days' ? 'E.g., 30' : 'E.g., 2'} 
              className="w-full bg-white/40 border border-white/60 text-[#2D3A47] px-4 py-3.5 text-sm font-black rounded-xl focus:outline-none focus:bg-white transition-all duration-200 mb-6 shadow-inner"
            />
